...
- **Service Account & IAM:**
  ...
  - The GitHub Actions deployment service account also needs permission to use
    the Compute Engine API and read/write the configured Terraform state bucket.
- **Secrets Management:**
  - Production `.env` remains on the VM host (`/opt/finai/.env`) and is never committed to Git, matching the repository guidelines.
...
| `admin_ssh_public_key` | `string` | *(Required)* | Public SSH key generated by the deployment workflow for the current run |
...
jobs:
  deploy:
    ...
    steps:
      ...
      - name: Generate temporary deployment key
        run: |
          ssh-keygen -t ed25519 -N '' -C "finai-github-actions" -f "$RUNNER_TEMP/finai_deploy_key"
          echo "SSH_PUBLIC_KEY=$(cat "$RUNNER_TEMP/finai_deploy_key.pub")" >> "$GITHUB_ENV"
          eval "$(ssh-agent -s)"
          ssh-add "$RUNNER_TEMP/finai_deploy_key"
      ...
      - name: Provision production infrastructure
        ...
        run: |
          terraform init \
            ...
            -backend-config="prefix=finai-production"
          terraform apply -auto-approve -input=false \
            -var="project_id=$GCP_PROJECT_ID" \
            -var="admin_ssh_public_key=$SSH_PUBLIC_KEY"
          echo "PROD_SSH_HOST=$(terraform output -raw instance_public_ip)" >> "$GITHUB_ENV"

      - name: Trust production host
        run: |
          for attempt in $(seq 1 60); do
            if ssh-keyscan -T 5 -H "$PROD_SSH_HOST" >> ~/.ssh/known_hosts 2>/dev/null; then
              exit 0
            fi
            sleep 10
          done
      ...
      - name: Wait for Docker bootstrap
        run: |
          for attempt in $(seq 1 60); do
            if ssh "$SSH_USER@$PROD_SSH_HOST" "docker compose version" >/dev/null 2>&1; then
              exit 0
            fi
            sleep 10
          done
      ...
      - name: Upload production Compose file
        run: |
          ssh "$SSH_USER@$PROD_SSH_HOST" "mkdir -p '$APP_DIR'"
          scp docker-compose.prod.yml "$SSH_USER@$PROD_SSH_HOST:${APP_DIR}/docker-compose.prod.yml"
      ...
      - name: Deploy image on GCP VM
        ...
        run: |
          printf '%s' "$GHCR_TOKEN" | ssh "$SSH_USER@$PROD_SSH_HOST" \
...
| :--- | :--- | :--- |
| `GCP_PROJECT_ID` | GCP project ID | Project where Terraform provisions the VM |
| `GCP_SA_KEY` | GitHub Environment secret | JSON credentials for Terraform and GCS state |
| `TF_STATE_BUCKET` | Existing GCS bucket name | Remote Terraform state bucket |
...
```yaml
  terraform:
    ...
    steps:
      ...
      - name: Generate temporary plan key
        run: |
          ssh-keygen -t ed25519 -N '' -C "finai-ci" -f "$RUNNER_TEMP/finai_ci_key"
          echo "CI_SSH_PUBLIC_KEY=$(cat "$RUNNER_TEMP/finai_ci_key.pub")" >> "$GITHUB_ENV"
      ...
      - name: Terraform Plan
        ...
        run: |
          terraform plan \
            ...
            -var="admin_ssh_public_key=$CI_SSH_PUBLIC_KEY" \
            -input=false
```
